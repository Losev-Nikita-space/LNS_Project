...
LNS_project/
 - config            # конфигурация
  -- config.yaml           # Конфигурация программы
 - device/                  # Основной модуль для работы с устройством
  -- __init__.py           # Делает каталог device Python-пакетом
  -- device_client.py      # Клиент
  -- udp_driver.py         # Драйвер для общения с устройством по UDP протоколу
  -- serial_driver.py      # Драйвер для общения с устройством по Setial-интерфейсу
  -- exceptions.py         # Исключения для обработки ошибок устройства
 - scripts/                 # Утилиты для развертывания и запуска
  ...
  -- service_control.sh    # Скрипт для администрирование сервиса мониторинга
